...
示例和自动生成的 next steps 默认使用长选项。`-m`、`-p`、`-d`、`-o`、`-t` 等短别名仍可用于交互式输入。
...
默认情况下，`DOMAIN/user` 会推断 `--scope-domain`、`--scope-base-dn` 和 `--base-dn`；连接默认使用 LDAP/389；设置 `--dmsa-name` 后，`--dns-hostname` 会按账号域名推断。对 `add` 来说，必须用 `--target-account` 显式选择 predecessor account，并用 `--principals-allowed` 指定可读取托管密码的主体。
...
- 使用 `dmsaforge plan ACTION ...`、`--dry-run` 或 `--plan` 校验参数并打印计划 LDAP 操作，不会打开 LDAP 连接。add 工作流的 JSON 报告会用规范 LDAP 属性名记录计划写入的 BadSuccessor 关键属性值。
...
- 未指定 `--method` 和 `--port` 时，先尝试 LDAP/389。只有传输连接失败时，dMSA Forge 才会继续尝试 LDAPS/636，并把候选尝试写入终端输出和 JSON/文本报告。认证失败会立即停止，不会再按 LDAPS 重试。单独传 `--port 636` 会推断为 LDAPS；同时指定 method 和 port 时才要求完全匹配。
...
- `delete` 必须显式传入 `--yes`。对象生命周期使用 `add`、`verify` 和 `delete`。
